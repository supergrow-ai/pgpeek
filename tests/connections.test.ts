...
describe("Connections API", () => {
  ...
  it("POST /api/connections returns the new connection id for auto-connect", async () => {
    const req = makeRequest("http://localhost:3000/api/connections", {
      method: "POST",
      body: { name: "Auto Connect DB", url: "postgresql://u:p@h:5432/d" },
    });
    const res = await POST(req);
    const data = (await parseJson(res)) as { id: number };
  ...
  it("POST /api/connections returns unique ids for each connection", async () => {
    const req1 = makeRequest("http://localhost:3000/api/connections", {
      method: "POST",
      body: { name: "DB 1", url: "postgresql://u:p@h:5432/d1" },
    });
    const req2 = makeRequest("http://localhost:3000/api/connections", {
      method: "POST",
      body: { name: "DB 2", url: "postgresql://u:p@h:5432/d2" },
    });
    const res1 = (await parseJson(await POST(req1))) as { id: number };
    const res2 = (await parseJson(await POST(req2))) as { id: number };

    expect(res1.id).not.toBe(res2.id);
  });

  it("newly created connection appears in GET list", async () => {
    const createReq = makeRequest("http://localhost:3000/api/connections", {
      method: "POST",
      body: { name: "New DB", url: "postgresql://u:p@h:5432/newdb" },
    });
