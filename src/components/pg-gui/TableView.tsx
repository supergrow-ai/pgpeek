...
export default function TableView({
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-[#e2e8f0] shrink-0">
        ...
        {onOpenQuery && (
          <Button
            ...
            className="h-7 text-[12px] text-slate-500 hover:text-slate-700"
            onClick={() => {
              const sql = buildSQL(schema, table, appliedSort, appliedFilters.map(({ column, operator, value }) => ({ column, operator, value })), limit, offset);
              onOpenQuery(sql);
            }}
          >
      ...
      {/* Query config panel — sort + filters grouped together */}
      {(draftSort || (showFilters && draftFilters.length > 0)) && (
        <div className="border-b border-[#e2e8f0] bg-[#fafbfd] shrink-0">
              ...
              <div className="flex items-center gap-2 px-4 py-1.5">
                ...
                {draftFilters.length > 0 && (
                  <button
                    ...
                    className="text-[12px] text-slate-400 hover:text-slate-600 ml-2 cursor-pointer"
                  >
                    Clear filters
                  </button>
                )}
          ...
          {/* Apply button — bottom-right of the config panel */}
          {isDirty && (
